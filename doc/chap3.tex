...
\begin{proof}
	...
	% もしこのような $\gamma \in \Delta$ がただ1つならば $\beta = \height \beta \gamma \in \Phi$ と言うことになり\hyperref[ax:root-system]{\textsf{\textbf{(Root-2)}}}に矛盾．よって $\beta \notin \Delta$ である．
...
\begin{proof}
	...
	\begin{description}
		...
		\end{align}
		より，$F_4$型となっている．Weyl群$\mathscr{W}$は，実は$D_4$型のWeyl群が正規部分群になっている．残りは$\Delta$の後ろ二つの単純ルートで生成できるので，$A_2$型のWeyl群と同型．よって，構造の一例は$(\mathbb{Z}_2^3 \rtimes \mathfrak{S}_4) \rtimes \mathfrak{S}_3 \simeq \mathbb{Z}_2^5 \rtimes (\mathfrak{S}_3 \times \mathfrak{S}_3)$であり，$D_4$型の$\Aut\Phi$とも同型である．
