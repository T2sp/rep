...
\begin{mylem}[label=lem:Weylgroup-dual]{}
	\begin{align}
...
$\gamma$ が正則ならば $\forall \alpha \in \Phi$ に対して $\rpair{\gamma}{\alpha} \neq 0$ なので，$\Phi = \Phi^+(\gamma) \amalg \bigl( -\Phi^+(\gamma) \bigr)$ （disjoint union）が成り立つ．
...
\begin{proof}
	\begin{description}
		...
		とおくと，仮定より
		\begin{align}
			\sum_{\alpha \in \Delta(\gamma)} s_\alpha \alpha = \sum_{\alpha \in \Delta(\gamma)} t_\alpha \alpha
		\end{align}
		が成り立つ．$\varepsilon \coloneqq \sum_{\alpha \in \Delta} s_\alpha \alpha$ とおくと，
		\textsf{\textbf{step2}}から
		\begin{align}
			0 \le \rpair{\varepsilon}{\varepsilon} = \sum_{\alpha,\, \beta \in \Delta(\gamma),\; \alpha \neq \beta} s_\alpha t_\beta \rpair{\alpha}{\beta} \le 0
		\end{align}
		が成り立つので $\varepsilon = 0$ だとわかる．よって
		\begin{align}
			0 &= \rpair{\gamma}{\varepsilon}
			= \sum_{\alpha \in \Delta(\gamma)} s_\alpha \rpair{\gamma}{\alpha}
			= \sum_{\alpha \in \Delta(\gamma)} t_\alpha \rpair{\gamma}{\alpha}
		\end{align}
		であり，$\forall \alpha \in \Delta(\gamma),\; s_\alpha = t_\alpha = 0$ が言えた．
...
\begin{mylem}[label=lem:Weylchamber-basic]{Weylの区画の基本性質}
	...
	\begin{enumerate}
		...
		\item 写像
		\begin{align}
			\{\, \text{\hyperref[def:base-root]{底}全体の集合}\, \} &\lto \{\, \text{\hyperref[def:Weylchamber]{Weylの区画}全体の集合}\, \}, \\
			\Delta &\lmto \mathfrak{C}(\Delta)
		\end{align}
		は全単射である．
		\item $\mathfrak{C}(\Delta) = \bigl\{\, \gamma \in \mathbb{E} \bigm| \forall \alpha \in \Delta,\; \rpair{\gamma}{\alpha} > 0 \,\bigr\}$
	\end{enumerate}
\end{mylem}
...
\begin{mylem}[label=lem:Weylchamber-Weylgroup]{Weylの区画とWeyl群の関係}
	\hyperref[def:decomposable]{正則}な任意の $\gamma \in \mathbb{E}$ および任意の $\Phi$ の\hyperref[def:base-root]{底} $\Delta$ を与える．このとき以下が成り立つ：
	\begin{enumerate}
		\item $\forall \sigma \in \mathscr{W}$ に対して $\sigma\bigl(\Delta(\gamma)\bigr) = \Delta \bigl( \sigma(\gamma) \bigr)$
		\item $\forall \sigma \in \mathscr{W}$ に対して $\sigma(\Delta)$ もまた $\Phi$ の\hyperref[def:base-root]{底}である．
