...
\begin{mydef}[label=def:free-Lie-alg]{自由Lie代数}
	集合$X$で生成される体$\mathbb{K}$上のLie代数$\mathfrak{g}$が$X$上自由であるとは，以下を満たすこと．
	\begin{description}
...
\begin{proof}
	\begin{description}
		\item[(一意性)] いつもの．
		\item[(存在)] $X$で生成される\hyperref[lem:univ-free-vec]{自由ベクトル空間}を$\mathbb{K}^{\oplus X}$とし，そのテンソル代数$T(\mathbb{K}^{\oplus X})$について，積をLie括弧とするLie代数と見る．$X$で生成される$T(\mathbb{K}^{\oplus X})$のLie部分代数を$\mathfrak{g}$とする．\\
		あとは\hyperref[def:univ-env-alg]{自由Lie代数の普遍性}を成立させれば良いが，その証明は\hyperref[sym-alg-univ]{対称代数の普遍性}の証明と同様で，$X \to \mathbb{K}^{\oplus X} \to T(\mathbb{K}^{\oplus X}) \to \mathfrak{g}$と縦に一つ伸びるだけである．
	\end{description}
\end{proof}
