...
\begin{mydef}[label=def:solvable-LieAlg]{可解Lie代数}
    ...
    \end{align}
    のこと．
    ある $n \ge 0$ に対して導来列が止まるとき，i.e. $D^n \mathfrak{g} = \{o\}$ が成り立つとき，Lie代数 $\mathfrak{g}$ は\textbf{可解} (solvable) であると言われる．
\end{mydef}
...
\begin{myprop}[label=prop:solvable-basic]{}
    ...
    \begin{enumerate}
        ...
        $\mathfrak{g}$ の任意の\hyperref[def:subLieAlg]{部分Lie代数}および $\Im f$ も可解である．
        \item $\mathfrak{g}$ のイデアル $\mathfrak{i}$ および\hyperref[def:quotient-LieAlg]{商代数} $\mathfrak{g}/\mathfrak{i}$ がどちらも可解ならば，$\mathfrak{g}$ 自身も可解である．
