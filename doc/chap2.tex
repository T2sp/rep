...
\begin{mytheo}[label=thm:Cartan-crit]{Cartanの判定条件}
	...
	このとき $\forall x \in \comm{\mathfrak{g}}{\mathfrak{g}},\; \forall y \in \mathfrak{g}$ に対して $\Tr (x \circ y) = 0$ が成り立つならば，$\mathfrak{g}$ は\hyperref[def:solvable-LieAlg]{可解}である．
\end{mytheo}
...
\begin{mydef}[label=def:g-module-hom]{$\mathfrak{g}$-加群の準同型}
    $\mathfrak{g}$ を\hyperref[ax:LieAlg]{Lie代数}，$V,\, W$ を \hyperref[ax:g-module]{ $\mathfrak{g}$-加群}とする．
...
% この小節の以降では有限次元表現を考える．
Lie代数 $\mathfrak{g}$ の表現 $\phi \colon \mathfrak{g} \lto \Lgl (V)$ は忠実であるとする．
...
さらに $\mathfrak{g}$ が\hyperref[def:semisimple-LieAlg]{半単純}ならば，定理\ref{thm:Cartan-crit}から $\phi(S) \subset \Lgl (\mathfrak{g})$ は\hyperref[def:solvable-LieAlg]{可解}であり，定理\ref{thm:semisimple-LieAlg-iff}の証明からこのとき $S_\beta = 0$ であること，i.e. $\beta$ が\hyperref[def:radical-bilinear]{非退化}であることが言える．

\begin{mylem}[label=lem:Casimir,breakable]{}
	$\{e_\mu\}$ を\hyperref[de:semisimple-LieAlg]{半単純Lie代数} $\mathfrak{g}$ の基底，
	$\beta \in L(\mathfrak{g},\, \mathfrak{g};\, \mathbb{K})$
	を対称かつ\hyperref[def:radical-bilinear]{非退化}な双線型形式であって $\forall x,\, y,\, z \in \mathfrak{g}$ に対して
	\begin{align}
...
\begin{proof}
	\begin{enumerate}
		...
		\begin{align}
			...
			&\IFF \forall (y^\nu)_{1 \le \nu \le \dim \mathfrak{g}} \in \mathbb{K}^{\dim \mathfrak{g}},\; \beta_{\mu\nu} x^\mu y^\nu = 0 \\
			&\IFF \beta_{\nu\mu} x^\mu = 0\quad (1 \le \forall \nu \le \dim \mathfrak{g}) \\
			&\IFF (x^\mu)_{1 \le \nu \le \dim \mathfrak{g}} \in \Ker \bigl[ \beta_{\mu\nu} \bigr]_{1 \le \mu,\, \nu \le \dim \mathfrak{g}} \subset \mathbb{K}^{\dim \mathfrak{g}}
		\end{align}
		が言える．ただし2つ目の同値変形で $\beta$ が対称であることを使った．したがって $\beta$ が非退化であることは $\Ker \bigl[ \beta_{\mu\nu} \bigr]_{1 \le \mu,\, \nu \le \dim \mathfrak{g}} = 0$ と同値であり，$\det \bigl[ \beta_{\mu\nu} \bigr]_{1 \le \mu,\, \nu \le \dim \mathfrak{g}} \neq 0$ が言える．
		よって $\bigl[ \beta_{\mu\nu} \bigr]_{1 \le \mu,\, \nu \le \dim \mathfrak{g}}$ の逆行列 $\bigl[ \alpha_{\mu}{}^\nu \bigr]_{1 \le \mu,\, \nu \le \dim \mathfrak{g}}$ が一意的に存在するので，
		$f_\mu \coloneqq e_\nu \alpha_\mu{}^\nu$ と定めると，
		\begin{align}
			\beta(e_\mu,\, f_\nu) 
		...
		\end{align}
		が成り立つ．
		\item $\ad(x)(e_\mu) \eqqcolon a_\mu{}^\nu e_\nu,\; \ad(f_\mu) \eqqcolon b_{\mu}{}^\nu f_\nu$ とおくと，
		\begin{align}
			a_\mu{}^\nu 
			&= a_\mu{}^\rho \delta_{\rho\nu} \\
			&= a_\mu{}^\rho \beta(e_\rho,\, f_\nu) \\
			&= \beta \bigl(\ad(x)(e_\mu),\, f_\nu\bigr) \\
			&= \beta \bigl(-\comm{e_\mu}{x},\, f_\nu\bigr) \\
			&= \beta \bigl(e_\mu,\, - \ad(x){f_\nu}\bigr) \\
			&= -b_\nu{}^\rho \beta \bigl(e_\mu,\, f_\rho\bigr) \\
			&= -b_\nu{}^\mu
		\end{align}
...
\begin{mydef}[label=def:Casimir]{忠実な表現のCasimir演算子}
	$\phi \colon \mathfrak{g} \lto \Lgl(V)$ を$\{e_\mu\}$ を\hyperref[de:semisimple-LieAlg]{半単純Lie代数} $\mathfrak{g}$ の基底，
	$\beta \in L(\mathfrak{g},\, \mathfrak{g};\, \mathbb{K})$
	を対称かつ\hyperref[def:radical-bilinear]{非退化}な双線型形式であって $\forall x,\, y,\, z \in \mathfrak{g}$ に対して
	\begin{align}
		\beta(x,\, \comm{y}{z}) = \beta (\comm{x}{y},\, z)
	\end{align}
	を充たすものとする．$\mathfrak{g}$ の基底 $\{e_\mu\}$ を一つ固定し，補題\ref{lem:Casimir}で構成した $\mathfrak{g}$ の基底 $\{f_\mu\}$ をとる．

	\begin{itemize}
		\item $\mathbb{K}$-線型変換
		\begin{align}
			\bm{c_\phi(\beta)} \colon V \lto V,\; v \lmto \sum_{\mu = 1}^{\dim \mathfrak{g}} \phi(e_\mu) \circ \phi(f_\mu) (v)
		\end{align}
		...
		\end{align}
		であるとき，$\beta,\, \phi$ の前Casimir演算子のことを\textbf{$\bm{\phi}$ のCasimir演算子} (Casimir element of $\phi$) と呼んで $\bm{c_\phi}$ と略記する．
	\end{itemize}
	
\end{mydef}

\begin{myprop}[label=prop:Casimir-basic]{Casimir演算子の性質}
	$\phi \colon \mathfrak{g} \lto \Lgl(V)$ を$\{e_\mu\}$ を\hyperref[de:semisimple-LieAlg]{半単純Lie代数} $\mathfrak{g}$ の基底，
	$\beta \in L(\mathfrak{g},\, \mathfrak{g};\, \mathbb{K})$
	を対称かつ\hyperref[def:radical-bilinear]{非退化}な双線型形式であって $\forall x,\, y,\, z \in \mathfrak{g}$ に対して
	\begin{align}
	...
	\begin{enumerate}
		...
		\end{align}
		が成り立つ．
		\item $\phi$ が\hyperref[def:faithful]{忠実な表現}でかつ $\phi$ が\hyperref[def:irr]{既約表現}ならば，\hyperref[def:Casimir]{Casimir演算子} $c_\phi \in \End V$ は $\mathfrak{g}$ の基底の取り方によらない．
	\end{enumerate}
\end{myprop}
...
\begin{proof}
	\begin{enumerate}
		...
		\begin{align}
			...
			&= \sum_{\mu=1}^{\dim \mathfrak{g}} \comm{\phi(x)}{\phi(e_\mu)} \circ \phi(f_\mu) + \sum_{\mu=1}^{\dim \mathfrak{g}} \phi(e_\mu) \circ \comm{\phi(x)}{\phi(f_\mu)} \\
			&= \sum_{\mu=1}^{\dim \mathfrak{g}} \ad\bigl(\phi(x)\bigr)\bigl(\phi(e_\mu)\bigr) \circ \phi(f_\mu) + \sum_{\mu=1}^{\dim \mathfrak{g}} \phi(e_\mu) \circ  \ad\bigl(\phi(x)\bigr)\bigl(\phi(f_\mu)\bigr) \\
			&= \sum_{\mu=1}^{\dim \mathfrak{g}} \phi\bigl(\ad(x)(e_\mu)\bigr) \circ \phi(f_\mu) + \sum_{\mu=1}^{\dim \mathfrak{g}} \phi(e_\mu) \circ  \phi\bigl(\ad(x)(f_\mu)\bigr) \\
			&= \sum_{\mu=1}^{\dim \mathfrak{g}} a_\mu{}^\nu \phi(e_\nu) \circ \phi(f_\mu) + \sum_{\mu=1}^{\dim \mathfrak{g}} b_\mu{}^\nu\phi(e_\mu) \circ \phi(f_\nu) \\
			&= 0
		\end{align}
		が言えた．
		...
		\begin{align}
			\Tr c_\phi 
			&= \sum_{\mu}^{\dim \mathfrak{g}} \Tr \bigl(\phi(e_\mu) \circ \phi(f_\mu) \bigr) \\
			&= \sum_{\mu}^{\dim \mathfrak{g}} \beta (e_\mu,\, f_\nu) \\
			&= \dim \mathfrak{g}
		\end{align}
		\item $\mathbb{K}$ が代数閉体でかつ $\phi$ が既約なので，\hyperref[col:Schur-closed]{代数閉体上のSchurの補題}から $c_\phi \colon V \lto V$ は $(\dim \mathfrak{g} / \dim V) \, \mathrm{id}_V$ に等しい．i.e. 基底によらない．
	\end{enumerate}
